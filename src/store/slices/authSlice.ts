...
export const loginUser = createAsyncThunk(
  ...
  async (credentials: LoginCredentials, { rejectWithValue }) => {
    try {
      const { data, error } = await authService.signIn(credentials.email || credentials.phone_number, credentials.password);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.fulfilled, (state, action) => {
        ...
        state.token = action.payload.session?.access_token || localStorage.getItem('access_token');
        state.refreshToken = action.payload.session?.refresh_token || localStorage.getItem('refresh_token');
        state.error = null;
      })
